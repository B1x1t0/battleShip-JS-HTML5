...
(function(config) {
    ...
    var Game = function() {
        ...
        this.Button = function(x, y, w, h, type, img) {
            ...
            this.fun = function(boat) {
                boat.orientation = boat.orientation == 'vertical'
                    ? 'horizontal'
                    : 'vertical';
            }
        };
    ...
    Game.prototype._drawBoats = function(boats) {
        for (var i = 0; i < boats.length; i++) {
            var boat = boats[i];
            boat.w = (boat.orientation == 'vertical')
                ? boat.width
                : boat.width * boat.size;
            boat.h = (boat.orientation == 'vertical')
                ? boat.height * boat.size
                : boat.height;
    ...
    Game.prototype._onClickAction = function(e) {
        ...
        var cells = this.boardEnemy.length;
        console.log("cells ", cells);
        //console.log(this.boats, this.boats.length);
        for (var i = cells - 1; i >= 0; i--) {
          console.log("miay");
            // draw shape onto ghost context
            if (this._clickOnCell(this.boardEnemy[i])) {
                var cell = this.boardEnemy[i];
                console.log(cell);
            }
    ...
    Game.prototype._clickOnBoat = function(boat) {
        if (this.mouse.x > boat.x && this.mouse.x < (boat.x + boat.w) && this.mouse.y > boat.y && this.mouse.y < (boat.y + boat.h)) {
            return true
        } else {
            return false;
        }
    ...
    Game.prototype._clickOnButton = function(button) {
        if (this.mouse.x > button.x && this.mouse.x < (button.x + button.w) && this.mouse.y > button.y && this.mouse.y < (button.y + button.h)) {
            return true
        } else {
            return false;
        }
    ...
    Game.prototype._clickOnCell = function(cell) {
        if (this.mouse.x > cell.x && this.mouse.x < (cell.x + cell.w) && this.mouse.y > cell.y && this.mouse.y < (cell.y + cell.h)) {
            return true
        } else {
            return false;
        }
    ...
    Game.prototype.drawshape = function(context, shape, fill) {
        ...
        if (shape.x > this.gameDetails.width || shape.y > this.gameDetails.height || shape.x + shape.w < 0 || shape.y + shape.h < 0) {
            if (shape.rotation != 0) {
                context.restore();
            }
    ...
    Game.prototype._checkBoatInCell = function(boat) {
        ...
        var maxX = boat.orientation == 'horizontal'
            ? (this.gameDetails.ceilWidth * (boat.size - 1)) + boat.x
            : boat.x;
        var maxY = (boat.orientation == 'vertical')
            ? (this.gameDetails.ceilHeight * (boat.size - 1)) + boat.y
            : boat.y;

        for (var i = 0; i < this.board.length; i++) {
            var ceil = this.board[i];
            if (posX >= ceil.x && posX < (ceil.x + this.gameDetails.ceilWidth) && posY >= ceil.y && posY < (ceil.y + this.gameDetails.ceilHeight)) {

                var lastPosition = (boat.orientation == 'vertical')
                    ? i + ((boat.size - 1) * 10)
                    : i + (boat.size - 1);
                ...
                this.ceilsInBoat = [];
                var sum = (boat.orientation == 'vertical')
                    ? this.gameDetails.columns
                    : 1;
                for (var pos = i; pos <= lastPosition; pos = pos + sum) {
                    this.ceilsInBoat.push(pos);
                    var lastCeil = this.board[pos];
                    if (lastCeil && maxX >= lastCeil.x && maxX < (lastCeil.x + this.gameDetails.ceilWidth) && maxY >= lastCeil.y && maxY < (lastCeil.y + this.gameDetails.ceilHeight)) {
                        boatInBoard = true;
                    }
                }
